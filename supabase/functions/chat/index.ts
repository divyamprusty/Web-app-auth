...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const userMessage = { session_id: sessionId, role: "user", content: body.message };
    ...
    if (msgErr) return new Response("Failed to save message", { status: 500, headers });
    ...
    if (content) await supabase.from("chat_messages").insert({ session_id: sessionId, role: "assistant", content });
    headers.set("Content-Type", "application/json");
